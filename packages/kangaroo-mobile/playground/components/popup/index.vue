...
<template>
  <div class="demo-popup">
    <demo-block card :title="t('basicUsage')">
      ...
      <yhm-popup v-model:show="showBasic" :style="{ padding: '64px' }">
        {{ t('content') }}
      </yhm-popup>
    </demo-block>
    ...
    <demo-block card :title="t('position')">
      <van-grid clickable>
        <van-grid-item icon="arrow-up" :text="t('buttonTop')" @click="showTop = true" />
        <van-grid-item icon="arrow-down" :text="t('buttonBottom')" @click="showBottom = true" />
        <van-grid-item icon="arrow-left" :text="t('buttonLeft')" @click="showLeft = true" />
        <van-grid-item icon="arrow" :text="t('buttonRight')" @click="showRight = true" />
      </van-grid>

      <yhm-popup v-model:show="showTop" position="top" :style="{ height: '30%' }" />
      <yhm-popup v-model:show="showBottom" position="bottom" :style="{ height: '30%' }" />
      <yhm-popup v-model:show="showLeft" position="left" :style="{ width: '30%', height: '100%' }" />
      <yhm-popup v-model:show="showRight" position="right" :style="{ width: '30%', height: '100%' }" />
    </demo-block>
    ...
    <demo-block card :title="t('roundCorner')">
      <van-cell :title="t('roundCornerCenter')" is-link @click="showRoundCornerCenter = true" />
      <yhm-popup v-model:show="showRoundCornerCenter" round :style="{ padding: '64px' }" />

      <van-cell :title="t('roundCornerBottom')" is-link @click="showRoundCornerBottom = true" />
      <yhm-popup v-model:show="showRoundCornerBottom" round position="bottom" :style="{ height: '30%' }" />
    </demo-block>

    <demo-block card :title="t('listenEvents')">
      <van-cell :title="t('clickEvents')" is-link @click="showClickEvents = true" />
      <yhm-popup
    ...
    <demo-block card :title="t('teleport')">
      ...
      <yhm-popup v-model:show="showGetContainer" teleport="body" :style="{ padding: '64px' }" />
    </demo-block>
  </div>
</template>
